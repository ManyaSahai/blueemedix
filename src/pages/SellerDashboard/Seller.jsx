...
function SellerDashboard() {
  ...
  const [updateOrderStatus, { isLoading: updateStatusLoading }] =
    useUpdateOrderStatusMutation();
  ...
  useEffect(() => {
    if (sellerOrdersData?.orders) {
      sellerOrdersData.orders.forEach((order) => {
        const orderId =
          typeof order === "string" ? order : order._id || order.id;
        ...
        if (!orderDetailsMap[orderId]) {
          fetch(`http://localhost:5000/api/orders/${orderId}`, {
            ...
            .catch((err) =>
              console.error(`Error fetching details for ${orderId}:`, err)
            );
        }
      });
    }
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      ...
      if (!validStatuses.includes(newStatus)) {
        setError(
          `Invalid status value: ${newStatus}. Status must be one of: ${validStatuses.join(
            ", "
          )}`
        );
        return;
          ...
          <TableContainer component={Paper} elevation={0}>
            ...
            <Table size={isMobile ? "small" : "medium"}>
              ...
              <TableBody>
                ...
                sellerOrdersData.orders.length > 0 ? (
                  sellerOrdersData.orders.map((order) => {
                    const isOrderObject =
                      typeof order !== "string" &&
                      order !== null &&
                      typeof order === "object";
                    const orderId = isOrderObject
                      ? order._id || order.id
                      : order;
                    ...
                    return (
                      <TableRow key={orderId}>
                        ...
                        <TableCell>
                          {isOrderObject && order.createdAt
                            ? new Date(order.createdAt).toLocaleDateString()
                            : isOrderObject && order.created_at
